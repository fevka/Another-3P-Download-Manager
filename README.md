...
On the **Updates** tab, paste a supported webpage URL returns the list of available update packages. The backend fetches the page HTML and extracts the FileCrypt container links (e.g. `filecrypt.cc/Container/...`) shown for each update.
